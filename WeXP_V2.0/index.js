...
ca:false, // authority chain for the clients
...
wss.on('connection', function (socket) {
    ...
    socket.on('message', msg => {
			console.log('message => ', msg.substr(0, 70));
        var message = JSON.parse(msg);
        console.log(`Connection ${sessionId} incoming: ${msg.substr(0, 70)}`);

        switch (message.id) { //message.id - register, message.name - 123..
            case 'register':
                ...
                user.sendMessage({id: 'registerResponse',
								response: 'accepted'});
                break;
